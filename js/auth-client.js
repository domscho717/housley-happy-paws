...
const HHP_Auth = {
  ...
  async handleSession(session) {
    ...
    if (profile) {
      ...
      // New user — default to client, create profile
  ...
  updateUIForUser() {
    // Update the view switcher dropdown — only show portals user has access to
    ...
    const displayName = this.currentUser?.profile?.full_name ||
                         this.currentUser?.email?.split('@')[0] || 'Welcome!';
    nameEls.forEach(el => el.textContent = displayName);

    // Update the Book Meet & Greet button to show "My Portal" if logged in
  ...
  canAccessPortal(portal) {
    ...
    if (this.currentRole === 'owner') return true; // owner sees everything
