...
(function() {
  ...
  _R._rwClientUpcoming=async function(sz){
    ...
    var lim=sz==='full'?8:2;
    try{var today=new Date().toISOString().split('T')[0];var{data}=await sb.from('booking_requests').select('id,service,preferred_date,preferred_time,scheduled_date,scheduled_time,estimated_total,status,pet_names').eq('client_id',u.id).in('status',['accepted','confirmed','modified']).gte('preferred_date',today).order('preferred_date').limit(lim);
      if(!data||!data.length)return'<div style="color:var(--mid);font-size:0.82rem;padding:8px 0">No upcoming appointments</div>';
      if(sz==='full'){
        return data.map(function(b){var d=new Date(b.preferred_date+'T12:00:00').toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'});var t=(typeof fmt12h==='function')?fmt12h(b.preferred_time||''):(b.preferred_time||'');var tcBanner='';if(b.status==='modified'&&b.scheduled_date){var nd=new Date(b.scheduled_date+'T12:00:00').toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'});var nt=b.scheduled_time?((typeof fmt12h==='function')?fmt12h(b.scheduled_time):b.scheduled_time):'';tcBanner='<div style="background:#fff8e1;border:1px solid #e67e22;border-radius:6px;padding:6px 8px;margin-bottom:6px;font-size:0.75rem"><span style="font-weight:700;color:#bf5d00">⏰ Time Change:</span> '+nd+(nt?' at '+nt:'')+' <a href="javascript:void(0)" onclick="event.stopPropagation();sTab(\'c\',\'c-appts\')" style="color:#e67e22;font-weight:700;margin-left:4px">Review</a></div>';}return tcBanner+'<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border);font-size:0.85rem"><div style="flex:1"><div style="font-weight:700">'+b.service+'</div><div style="font-size:0.75rem;color:var(--mid)">'+d+(t?' · '+t:'')+(b.pet_names?' · '+b.pet_names:'')+'</div></div><div style="text-align:right"><div style="font-weight:600;color:'+(b.status==='modified'?'#e67e22':'var(--forest)')+'">$'+(b.estimated_total||0).toFixed(2)+'</div><div style="font-size:0.65rem;color:'+(b.status==='modified'?'#e67e22':'var(--forest)')+';text-transform:uppercase">'+(b.status==='modified'?'Time Change':b.status)+'</div></div></div>';}).join('');
      }
      return data.map(function(b){var d=new Date(b.preferred_date+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'});var label=b.status==='modified'?'⏰ '+b.service:b.service;return'<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:0.78rem"><span style="font-weight:600;'+(b.status==='modified'?'color:#e67e22':'')+'">'+label+'</span><span style="color:var(--mid)">'+d+'</span></div>';}).join('');
    }catch(e){return'';}
  };
